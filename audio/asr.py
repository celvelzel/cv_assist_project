...
class ASREngine:
    ...
    def transcribe_file(self, audio_path: str, return_timestamps: bool = False) -> dict:
        try:
            ...
            output = {"text": text, "language": out_lang}
    ...
    def parse_command_with_vision(
        ...
        if llm_parser is not None and frames:
            try:
                logger.debug("Attempting LLM vision parsing: text='%s', frames=%s", text, len(frames))
                result = llm_parser.parse_with_vision(text, frames)
                if result and "target" in result:
                    target = result["target"].strip()
                    if target:
                        logger.info("LLM vision parsing succeeded: '%s' -> '%s'", text, target)
                        return target
                else:
